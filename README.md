...
I wanted to make a program in C#, that could read the Raw sectors on any attached harddrive. It turns out this was really simple, except, to make it portable / work for any drive, one needed to know the Sector size. 
...
It also turns out, that the amount of help for this function on the internet for use in C# is ... close to non-existent. So I decided to try and map at least some of the DeviceIOControl functionalitites in a library.
...
The basic idea, is to take a handle to a Windows device (f.ex. a harddrive, CD rom drive, tape drive, disk changer, USB device, Bluetooth device, filesystem ... ), and then make it easy to send control commands to this device.
...
A simple scenario is to open the CDRom tray. First we need a handle to the CDRom device. A great tool to discover devices is the [Sysinternals Winobj][1] program. My CD drive is named: "\\.\CdRom0". I then use the [Win32 CreateFile()][2] to open the device. I do this with Win32 instead of FileStream as FileStream doesn't support devices - sadly.
...
In the code above, we have all the boilerplate as well (such as the CreateFile dll import from [pinvoke.net][3]). After we get the handle, we use it in the DeviceIOControlWrapper, and simple call some Storage methods on it. For CD drives, we can eject and 'load' media. I do that, and voilá, CD tray opens and closes.

----------

**Reading out information about harddrives**

The thing that got me started, was reading out sector sizes. So, lets do that. Working frome code above, we open a handle to PhysicalDriveN instead (all harddrives attached are mapped this way).
